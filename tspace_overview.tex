...
\begin{tikzpicture}[
  ...
  \draw (av.2) edge [arrows={-Stealth[harpoon]},"Pipeline\texttt{\string[Dataframe\string]}"] (cr.178)
        (cr.2) edge [arrows={-Stealth[harpoon]},"level"'] (ag.178);
  \draw (ag.182) edge [arrows={-Stealth[harpoon]},"raw"] (cr.358)
        (cr.182) edge [arrows={-Stealth[harpoon]},"data"'] (av.358);
  \draw (cr) edge ["raw", "level"'] (db);
  \draw (kv) edge [<->,"KvaserCAN"] (av);
  \draw (av) edge [<->,densely dashed,"RemoteCAN"] (cl);
  \draw (v) edge [<->,"CAN/UDP"] (kv);
  \draw (cl.227) edge [<->, densely dash dot dot,bend right,"OTA"] node[hw,solid,very near end, sloped, below,]{TBox} (v.133);
\end{tikzpicture}
\end{document}
